...
R package for building OLS and performing diagnostics tests olsrr R package for calibrating geographical weighted family of models GWmodel R package for multivariate data visualisation and analysis corrplot Spatial data handling sf Attribute data handling tidyverse, especially readr, ggplot2 and dplyr Choropleth mapping tmap
...
In the section, we will learn how to use statistical graphics functions of ggplot2 package to perform EDA. \## 6.7.1 EDA using statistical graphic We can plot the distribution of SELLING_PRICE by using appropriate Exploratory Data Analysis (EDA) as shown in the code chunk below.
...
In this section, we will learn how to building hedonic pricing models for condominium resale units using lm() of R base. \## 6.8.1 Simple Linear Regression Method First, we will build a simple linear regression model by using SELLING_PRICE as the dependent variable and AREA_SQM as the independent variable.
...
```{r corrplot, fig.height = 5, fig.width = 5}
...
In this section, we are going to learn how to modelling hedonic pricing using both the fixed and adaptive bandwidth schemes \## 6.9.1 Building Fixed Bandwidth GWR Model \### 6.9.1.1 Computing fixed bandwith In the code chunk below bw.gwr() of GWModel package is used to determine the optimal fixed bandwidth to use in the model. Notice that the argument adaptive is set to FALSE indicates that we are interested to compute the fixed bandwidth.

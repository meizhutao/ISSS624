...
# Visualizing the spatial distribution of functional and non-functional water points rate
...
# Thematic Mapping for functional water point rate
...
Now lets narrow down to reveal spatial patterns of Functional water points rate.
...
### Maron's I test for funtional water points rate
...
Conclusion: Moran I statistic is positive (0.54), p-value is 2.2e-16, which is less then 0.001, which indicates that we can reject the Null Hypothesis. The spatial distribution of high values in the dataset is more spatially clustered than would be expected if underlying spatial processes were truly random.
...
Now, let's calculate nga.localMI for functional water points rate.
...
Now, let's plot local Moran's I values for functional water points rate.
...
The following code chunks are for functional water points rate.
...
```
Conclusion: 
local Moran's I values (4-5) is positive (I>0), and p-values is less than 0.001: Clustered, The spatial distribution of high values of functional water point rate in the dataset is more spatially clustered.

# Creating a LISA Cluster Map for functional water point rate
...
Notice that the plot is split in 4 quadrants. The top right corner belongs to areas that have high functional water points rate and are surrounded by other areas that have the average level of functional water points rate.
...
# Computing GI Statistics for funtional water point rate
...
## Mapping Gi values with adaptive distance weight
```{r}
func <- qtm(nga_wp, "pct_functional")
...
# Thematic Mapping for non-functional water point rate

## Visualising Regional Development Indicator

Now, we are going to prepare a basemap and a choropleth map showing the distribution of functional and non-functional water points by using qtm() of tmap package.

Now lets narrow down to reveal spatial patterns of non-Functional water points rate.
...
```{r}
wm_q <- poly2nb(nga_wp_nonfunctional, 
                queen=TRUE)
summary(wm_q)
```
...
### Maron's I test for non-funtional water points rate
...
Conclusion: Moran I statistic is positive (0.48), p-value is 2.2e-16,which is less then 0.001, which indicates that we can reject the Null Hypothesis. The spatial distribution of high values in the dataset is more spatially clustered than would be expected if underlying spatial processes were truly random.
...
localMI_non_functional <- localmoran(nga_wp_nonfunctional$`pct_non-functional`, rswm_q)
...
Now, let's plot local Moran's I values for non-functional water points rate.
...
The code chunks below produce a choropleth map of Moran's I p-values by using functions of tmap package. 
The following code chunks are for non-functional water points rate.
...
# Creating a LISA Cluster Map for non-functional water point rate
...
Notice that the plot is split in 4 quadrants. The top right corner belongs to areas that have high non-functional water points rate and are surrounded by other areas that have the average level of non functiaonal water points rate.
...
# Computing GI Statistics for non-functional water point rate

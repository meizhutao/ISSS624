...
```
```{r}
tmap_arrange(pct_handpump,pct_mechanizedpump, pct_usage_less_1000,pct_usage_equal_more_1000,asp=2, ncol=2)
```
We can see both pct_handpump & pct_usage_less_1000 and pct_mechanizedpump & pct_usage_equal_more_1000 are showing similar choropleth map, they might be highly correlated. We will do Correlation Analysis at later part.
...
cluster_vars.cor = cor(nga_wp_var[,2:10])
...
If coefficient is between 0.7 to 1, it suggests there is a strong correlation. pct_usage_less_1000 & pct_handpump, pct_usage_more_1000 &pct_mechanizedpump,pct_handpump & pct_mechanizedpump are having strong correlation with . Hence, we remove pct_usage_less_1000, pct_usage_more_1000, pct_mechanizedpump.
...
```{r}
nga_wp <- nga_wp %>%
  select("shapeName","wpt functional","wpt non-functional", "pct_functional","pct_non-functional", "pct_handpump",  "pct_rural")
cluster_vars <- nga_wp %>%
  st_set_geometry(NULL) 
...
```
By examine the gap statistic graph, the 10-cluster gives the largest gap statistic and should be the next best cluster to pick. Here, we select 4, because the slope is relatively flat after 4 cluster.
...
With closed examination of the dendragram above, we have decided to retain 4 clusters.
...
```{r}
hclust.map <-qtm(nga_wp_cluster, "HCLUSTER")+ tm_borders(alpha = 0.5)+ 
  tm_layout(main.title = "Hierarchical clustering",
            ...
            main.title.fontface = "bold",
            frame = TRUE)
tmap_arrange(hclust.map, nrow = 1,ncol = 1)
```
...
```{r p1, fig.height= 10, fig.width = 15}
...
```{r p2, fig.height= 10, fig.width = 15}
p2<-plot(nga_wp_sp, border=gray(.5))
p2<-plot.mst(nga_wp.mst, 
         coordinates(nga_wp_sp), 
         col="blue", 
         cex.lab=0.7, 
         cex.circles=0.005, 
...
```{r p3, fig.height = 20, fig.width = 15}
p3 <- plot(nga_wp_sp, border=gray(.5))
p3 <- plot(clust4, 
     coordinates(nga_wp_sp), 
     cex.lab=.7,
...
shclust.map <-qtm(nga_wp_spatialcluster, "SP_CLUSTER")+   
  ...
  tm_layout(main.title = "Spatially constrained",
            ...
            main.title.fontface = "bold",
            frame = TRUE)
tmap_arrange(shclust.map, nrow = 1,ncol = 1)
```
...
ggparcoord(data = nga_wp_spatialcluster,
  ...
  theme(axis.text.x = element_text(angle = 90))
```
The parallel coordinate plot above reveals that in Cluster 1 & 2, it tends to have higher non-functional water points and rate compared to functional water points and rate at rural area, which might be a concern.
...
nsh_ngeo.map <- qtm(nga_wp_ngeo_cluster, "NGCLUSTER")+   
  tm_borders(alpha = 0.5) +
  tm_layout(main.title = 
              "Non-spatially constrained:ClustGeo",
            main.title.position = "center",
            main.title.size = 0.8,
            legend.height = 0.5, 
            legend.width = 0.4,
            ...
            main.title.fontface = "bold",
            frame = TRUE)
tmap_arrange(nsh_ngeo.map, nrow = 1,ncol = 1)
```
...
```{r}
sh_ngeo.map <- qtm(nga_wp_Gcluster, "GCLUSTER")+   
  tm_borders(alpha = 0.5) +
  tm_layout(main.title = "Spatially constrained :ClustGeo",
            main.title.position = "center",
            main.title.size = 0.8,
            legend.height = 0.5, 
            legend.width = 0.4,
            ...
            main.title.fontface = "bold",
            frame = TRUE)
tmap_arrange(sh_ngeo.map, nrow = 1,ncol = 1)
```
...
For easy comparison, it will be better to place the all the clustering maps together.
```{r}
tmap_arrange(hclust.map, shclust.map,nsh_ngeo.map, sh_ngeo.map,
             asp=NA, ncol=2)
```

We can observe that spatially constrainted clustering analysis does not clearly define variables clustering segmentation compared to other clustering analysis method. 
Hierarchy Cluster Analysis provides better clustering segmentation compared to other clustering analysis method. 
In this study, we will select ClustGeo method for non-spatially constrained hierarchical cluster analysis and spatially constrained cluster analysis.
It gives the dissimilarities in the attribute/clustering variable space. D1, on the other hand, gives the dissimilarities in the constraint space. 
...
nga_wp_ngeo_cluster %>% 
  ...
  summarise(mean_wpt_functional = mean(wpt.functional),
            ...
            mean_pct_nonfunctional = mean(pct_functional),
            mean_pct_nonfunctional = mean(pct_non.functional),
            mean_pct_handpump = mean(pct_handpump),
            mean_pct_rural = mean(pct_rural))

```
The above 2 tables reveal that in Cluster 2&3, it tends to have higher non-functional water points and rate compared to functional water points and rate at rural area, which might be a main concern.
```{r}
nga_wp_Gcluster %>% 
  st_set_geometry(NULL) %>%
  group_by(GCLUSTER) %>%
  summarise(mean_wpt_functional = mean(wpt.functional),
            ...
            mean_pct_rural = mean(pct_rural))
```
The above 2 tables reveal that in Cluster 3&4, it tends to have higher non-functional water points and rate compared to functional water points and rate at rural area, which might be a main concern.

We also can refer to "Multiple Parallel Coordinates Plots of non-spatially constrained hierarchical clustering variables by Cluster" and "Multiple Parallel Coordinates Plots of spatially constrained hierarchical clustering variables by Cluster". In summary, southwest of Nigeria  has the highest risk of suffering from insufficient water due to higher non-functional water points and water point rate.

...
cluster_vars.cor = cor(nga_wp_var[,4:10])
...
nga_wp_ngeo_cluster %>% 
  ...
  summarise(mean_wpt_functional = mean(wpt.functional),
            ...
            mean_pct_mechanizedpump = mean(pct_mechanizedpump),mean_pct_rural = mean(pct_rural))
```
```{r}
nga_wp_Gcluster %>% 
  st_set_geometry(NULL) %>%
...
We also can refer to "Multiple Parallel Coordinates Plots of non-spatially constrained hierarchical clustering variables by Cluster" and "Multiple Parallel Coordinates Plots of spatially constrained hierarchical clustering variables by Cluster". In summary, in Cluster 3&4, it tends to have higher non-functional water points and rate compared to functional water points and rate at rural area, which might be a main concern, we may consider to drive action starting from cluster 3&4.
